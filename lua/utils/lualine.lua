...
function M.status(icon, status)
    ...
    return {
        ...
        color = function()
            return gvimconf.utils.ui.fg(colors[status()] or colors.ok)
        end,
    }
end
...
function M.cmp_source(name, icon)
    icon = icon or gvimconf.utils.config.icons.kinds[name:sub(1, 1):upper() .. name:sub(2)]
    local started = false
...
function M.pretty_path(opts)
    ...
    return function(self)
        ...
        path = gvimconf.utils.norm(path)
        local root = gvimconf.utils.root.get({ normalize = true })
        local cwd = gvimconf.utils.root.cwd()
...
function M.root_dir(opts)
    opts = vim.tbl_extend("force", {
        ...
        icon = "󱉭 ",
        color = function()
            return gvimconf.utils.ui.fg("Special")
        end,
    }, opts or {})
