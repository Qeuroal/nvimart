...
function M.decorate(name, mod)
    ...
    setmetatable(mod, {
        __call = function(_, ...)
            ...
            gvimconf.utils.deprecate("gvimconf.utils." .. name, to)
        ...
        __index = function(_, k)
            if M.moved[name][k] then
                ...
                gvimconf.utils.deprecate("gvimconf.utils." .. name .. "." .. k, to)
                local ret = vim.tbl_get(_G, unpack(vim.split(to, ".", { plain = true })))
                return ret
            end
            return nil
...
function M.lazygit()
    gvimconf.utils.deprecate("gvimconf.utils.lazygit", "Snacks.lazygit")
    return Snacks.lazygit
end

function M.toggle()
    gvimconf.utils.deprecate("gvimconf.utils.toggle", "Snacks.toggle")
    return {
        map = function() end,
        wrap = function()
            return {}
