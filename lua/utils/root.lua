...
function M.detectors.lsp(buf)
    ...
    local clients = gvimconf.utils.lsp.get_clients({ bufnr = buf })
...
function M.realpath(path)
    ...
    return gvimconf.utils.norm(path)
end
...
function M.setup()
    vim.api.nvim_create_user_command("LazyRoot", function()
        gvimconf.utils.root.info()
    end, { desc = "vimart roots for the current buffer" })
...
function M.get(opts)
    ...
    return gvimconf.utils.is_win() and ret:gsub("/", "\\") or ret
end
