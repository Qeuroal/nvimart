...
local ft_group = vim.api.nvim_create_augroup("VimartFt", { clear = true })
...
M.get_root = function()
    ...
    local pattern = gvimconf.chdir_root_pattern
...
M.group_map = function(group, opt)
    if not opt then
        opt = {}
    end
    ...
    for desc, keymap in pairs(group) do
        ...
        local default_option = vim.tbl_extend("force", { desc = desc, noremap = true, nowait = true, silent = true, }, opt)
...
M.update = function()
    vim.system({ "git", "pull" }, { cwd = vim.fn.stdpath "config", text = true }, function(out)
        if out.code == 0 then
            vim.notify "nvimart update to date"
        else
            vim.notify("nvimart update failed: " .. out.stderr, vim.log.levels.WARN)
        end
    end)
end
