...
function M.formatexpr()
    if gvimconf.utils.has("conform.nvim") then
...
function M.info(buf)
    ...
    gvimconf.utils[enabled and "info" or "warn"](
        table.concat(lines, "\n"),
...
function M.format(opts)
    ...
    if not done and opts and opts.force then
        gvimconf.utils.warn("No formatter available", { title = "gvimconf.utils" })
    end
end
...
function M.health()
    ...
    local has_extra = vim.tbl_contains(Config.spec.modules, "plugin.extras.lsp.none-ls")
    if has_plugin and not has_extra then
        gvimconf.utils.warn({
            "`conform.nvim` and `nvim-lint` are now the default formatters and linters in gvimconf.",
            "",
            "You can use those plugins together with `none-ls.nvim`,",
            "but you need to enable the `plugin.extras.lsp.none-ls` extra,",
...
function M.snacks_toggle(buf)
    return Snacks.toggle({
        ...
        get = function()
            ...
            return gvimconf.utils.format.enabled()
        end,
        set = function(state)
            gvimconf.utils.format.enable(state, buf)
        end,
    })
end
