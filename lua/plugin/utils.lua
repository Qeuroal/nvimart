...
M.about = function()
    ...
    render("", 1)
    render("Welcome to NVIMART.", 2)
    render("", 3)
    render("Author: Qeuroal", 4)
    render("", 5)
    render("Url: https://github.com/qeuroal/nvimart.git", 6)
    render("", 7)
    render(string.format("Copyright © 2022-%s Qeuroal", os.date "%Y"), 8)
...
M.check_icons = function()
    ...
    local new_line = line()
    local row
    for name, icon in require("utils.basic").ordered_pair(gvimconf.symbols) do
...
-- Set up colorscheme and gvimconf.colorscheme, but does not take care of lualine
...
M.colorscheme = function(colorscheme_name)
    gvimconf.colorscheme = colorscheme_name

    local colorscheme = gvimconf.colorschemes[colorscheme_name]
...
M.select_colorscheme = function()
    ...
    local function picker(opts)
        ...
        local colorschemes = gvimconf.colorschemes
        local suffix_current = " (current)"
        local results = { gvimconf.colorscheme .. suffix_current }
        for name, _ in require("utils.basic").ordered_pair(colorschemes) do
            if name ~= gvimconf.colorscheme then
